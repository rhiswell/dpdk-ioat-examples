...
static int test_enqueue_copies(int dev_id) {
    ...
    do {
        ...
        for (i = 0; i < length; i++) src_data[i] = rand() & 0xFF;

        if (rte_ioat_enqueue_copy(dev_id, src->buf_iova + src->data_off,
                                  dst->buf_iova + dst->data_off, length,
    ...
    do {
        ...
        for (i = 0; i < RTE_DIM(srcs); i++) {
            ...
            for (j = 0; j < length; j++) src_data[j] = rand() & 0xFF;

            if (rte_ioat_enqueue_copy(
                    dev_id, srcs[i]->buf_iova + srcs[i]->data_off,
                    dsts[i]->buf_iova + dsts[i]->data_off, length,
    ...
    for (i = 0; i < RTE_DIM(length); i++) {
        ...
        for (j = 0; j < length[i]; j++) {
            ...
            if (dst_data[j] != pat_byte) {
                PRINT_ERR(
                    "Error with fill operation (length = %u): got (%x), not "
                    "(%x)\n",
                    length[i], dst_data[j], pat_byte);
                return -1;
            }
        }
    }
...
#define IOAT_TEST_RINGSIZE 512
    ...
    if (p.ring_size != expected_ring_size[dev_id]) {
        PRINT_ERR(
            "Error, initial ring size is not as expected (Actual: %d, "
            "Expected: %d)\n",
    ...
    pool =
        rte_pktmbuf_pool_create("TEST_IOAT_POOL", 256, /* n == num elements */
                                32,                    /* cache size */
                                0,                     /* priv size */
                                2048,                  /* data room size */
                                info.socket_id);
    ...
    for (i = 0; i < 100; i++) {
        ...
        if (test_enqueue_copies(dev_id) != 0) goto err;
    ...
    for (i = 0; i < 100; i++) {
        ...
        if (test_enqueue_fill(dev_id) != 0) goto err;
